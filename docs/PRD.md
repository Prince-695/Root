...
- Users run it via package runners (`pnpm dlx root-scaffold@latest …`).
...
| Global binary / awkward install UX | `pnpm dlx root-scaffold@latest` |
...
| shadcn/ui | Root |
|---|---|
| `pnpm dlx shadcn@latest init` | `pnpm dlx root-scaffold@latest init` |
| `components.json` | `root.json` |
| `pnpm dlx shadcn@latest add button` | `pnpm dlx root-scaffold@latest add route post` |
...
- Runs `pnpm dlx root-scaffold@latest init` in an empty folder, picks Postgres + Prisma, ships
...
**Trigger:** `pnpm dlx root-scaffold@latest init` from any directory.
...
4. Optional CLI skip: `pnpm dlx root-scaffold@latest init my-api` creates `./my-api` without the folder prompt
...
**Trigger:** `pnpm dlx root-scaffold@latest add …` when `root.json` exists.
...
bunx root-scaffold@latest init
```
...
```bash
pnpm dlx root-scaffold@latest init
pnpm dlx root-scaffold@latest init --yes          # use defaults where safe (advanced)
pnpm dlx root-scaffold@latest init --dry-run      # preview file tree, write nothing
pnpm dlx root-scaffold@latest init --verbose
```
...
### 12.2 Example Transaction — `pnpm dlx root-scaffold@latest add route post`
