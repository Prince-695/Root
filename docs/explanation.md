...
| **Package / CLI** | A program you run with a short command. You run Root with `npx rooot@latest` (package and command: `rooot`). |
...
| Goal | Command |
|---|---|
| Run latest from npm (pnpm) | `pnpm dlx rooot@latest <command>` |
| Run latest from npm (npm) | `npx rooot@latest <command>` |
| Init with defaults | `pnpm dlx rooot@latest --yes init` |
| Init into a named folder | `pnpm dlx rooot@latest init my-api` |
| Skip installing deps during init | `pnpm dlx rooot@latest init --skip-install` |
| Local from this monorepo | `pnpm root-cli <command>` (after `pnpm build`) |
| From a downloaded release `.tgz` | `pnpm dlx ./rooot-0.1.0.tgz --yes init my-api` |

> Invoke with `npx rooot@latest` (also `pnpm dlx` / `yarn dlx` / `bunx`). Local monorepo: `pnpm root-cli`.
...
pnpm dlx rooot@latest init
# or
pnpm dlx rooot@latest --yes init my-api
```
...
npx rooot@latest remove resource post
```
...
```bash
pnpm dlx rooot@latest doctor
pnpm dlx rooot@latest doctor --strict
```
...
```bash
pnpm dlx rooot@latest --dry-run add resource comment
```
...
2. `pnpm dlx rooot@latest --yes init` (or answer the wizard)  
3. `pnpm dlx rooot@latest add auth`  
4. `pnpm dlx rooot@latest add resource note`  
5. `cp .env.example .env` → set secrets → `pnpm install` → `pnpm dev`  
6. Sign up → sign in → create a note with the token → list notes  
7. `pnpm dlx rooot@latest doctor`  
...
- Invoke: `npx rooot@latest` · capabilities: `add resource`, `add auth`, …  
